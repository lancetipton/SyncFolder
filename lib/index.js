...
const validateConfig = config => {
  ...
  if(!dirKeys.length)
    throw new Error(`Invalid Config: 'dir.<directory key>' attribute must be defined!`)
  
  dirKeys.map(key => {
    const dirConf = config.dirs[key]
    if(!dirConf.src || typeof dirConf.src !== 'string')
      throw new Error(`Invalid Config: 'directory.src' attribute must be a string!`)
    else if(!dirConf.dest || !Array.isArray(dirConf.dest))
      throw new Error(`Invalid Config: 'directory.dest' attribute must be an array!`)
    else if(dirConf.dest.length === 0)
      throw new Error(`Invalid Config: 'directory.dest' is empty, not destinations to sync!`)
  })
